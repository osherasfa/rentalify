...
// Generic descriptors the LLM sometimes emits as a "neighborhood" (e.g. "מרכז
// הוד השרון" -> neighborhood "מרכז"). These are relative words, not places —
// geocoding them lands on whatever unrelated gazetteer point happens to share the
// name (the bundled "מרכז" entry sat in Eilat, ~280km off). Never treat them as a
// neighborhood; fall through to the city instead. normPlace-normalized.
const GENERIC_HOODS = new Set([
  "מרכז", "מרכז העיר", "העיר", "מרכז השכונה",
  "צפון", "דרום", "מזרח", "מערב", "צפון העיר", "דרום העיר",
]);
...
function lookupHood(hoodText, cityText) {
  ...
  if (GENERIC_HOODS.has(key)) return null; // "center"/"north"/... is not a place
...
/**
 * `city` is the gazetteer's parent city for a matched neighborhood, so callers
 * can backfill location.city when the LLM only gave a neighborhood name.
 * @returns {{lat:number,lng:number,source:"neighborhood"|"city",city:string|null}|{lat:null,lng:null,source:"none",city:null}}
 */
export function geocode(cityText, hoodText) {
  ...
  if (h) return { lat: h.lat, lng: h.lng, source: "neighborhood", city: h.city || null };
  // A city/town match from the city field.
  const c = lookupCity(cityText);
  if (c) return { lat: c.lat, lng: c.lng, source: "city", city: null };
  ...
  if (c2) return { lat: c2.lat, lng: c2.lng, source: "city", city: null };
  // ...or a quarter name sitting in the city field.
  const h2 = lookupHood(cityText, null);
  if (h2) return { lat: h2.lat, lng: h2.lng, source: "neighborhood", city: h2.city || null };
  return { lat: null, lng: null, source: "none", city: null };
}
